/* ===========================================================================
   ...
   SCHEMA NAME: all objects live in the [amazon_webapp] schema. To use a
   different name, find-replace the single token "amazon_webapp" throughout
   this file (it appears only as the schema qualifier + the CREATE SCHEMA guard;
   table/column/constraint names do not contain it).

   DESIGN RULES APPLIED
     • All objects live in the [amazon_webapp] schema.
   ...
   AMBIGUITY DECISIONS (each also commented inline):
     ...
     • The denormalised *_count columns on scans/analytics_runs are kept (the app
       reads them directly); amazon_webapp.scan_stats VIEW is provided as the
...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
/*
DROP TABLE IF EXISTS amazon_webapp.brand_analytics_items;
DROP TABLE IF EXISTS amazon_webapp.brand_analytics_runs;
DROP TABLE IF EXISTS amazon_webapp.analytics_candidates;
DROP TABLE IF EXISTS amazon_webapp.analytics_catalog_rows;
DROP TABLE IF EXISTS amazon_webapp.analytics_runs;
DROP TABLE IF EXISTS amazon_webapp.scan_candidates;
DROP TABLE IF EXISTS amazon_webapp.scan_results;
DROP TABLE IF EXISTS amazon_webapp.scan_amazon_rows;
DROP TABLE IF EXISTS amazon_webapp.scan_catalog_rows;
DROP TABLE IF EXISTS amazon_webapp.scans;
DROP TABLE IF EXISTS amazon_webapp.pair_library_ids;
DROP TABLE IF EXISTS amazon_webapp.pair_library_entries;
DROP TABLE IF EXISTS amazon_webapp.brand_entities;
DROP TABLE IF EXISTS amazon_webapp.asin_identifier_overrides;
DROP TABLE IF EXISTS amazon_webapp.blacklisted_pairs;
DROP TABLE IF EXISTS amazon_webapp.verified_items;
DROP TABLE IF EXISTS amazon_webapp.attribute_caches;
DROP TABLE IF EXISTS amazon_webapp.global_asin_caches;
DROP TABLE IF EXISTS amazon_webapp.amazon_imports;
DROP TABLE IF EXISTS amazon_webapp.keepa_imports;
DROP TABLE IF EXISTS amazon_webapp.abbreviation_entries;
DROP TABLE IF EXISTS amazon_webapp.settings;
GO
*/
...
/* ---------------------------------------------------------------------------
   1.  SCHEMA
   --------------------------------------------------------------------------- */
IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'amazon_webapp')
    EXEC('CREATE SCHEMA amazon_webapp');
GO
...
CREATE TABLE amazon_webapp.settings (
...
CREATE TABLE amazon_webapp.abbreviation_entries (          -- was abbreviation_library
    id            INT            IDENTITY(1,1) NOT NULL,
...
CREATE TABLE amazon_webapp.keepa_imports (
    id            INT            IDENTITY(1,1) NOT NULL,
...
CREATE TABLE amazon_webapp.amazon_imports (
    id            INT            IDENTITY(1,1) NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
    data_json     NVARCHAR(MAX)  NOT NULL,
    imported_at   DATETIME2      NOT NULL DEFAULT GETUTCDATE(),
...
CREATE TABLE amazon_webapp.global_asin_caches (            -- was global_asin_cache
    id            INT            IDENTITY(1,1) NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
    data_json     NVARCHAR(MAX)  NOT NULL,
...
CREATE TABLE amazon_webapp.attribute_caches (              -- was attribute_cache
    id              INT           IDENTITY(1,1) NOT NULL,
...
CREATE TABLE amazon_webapp.verified_items (
    id            INT            IDENTITY(1,1) NOT NULL,
    upc           NVARCHAR(20)   NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
    data_json     NVARCHAR(MAX)  NOT NULL,
...
CREATE TABLE amazon_webapp.blacklisted_pairs (
    id               INT           IDENTITY(1,1) NOT NULL,
    upc              NVARCHAR(20)  NOT NULL,
    asin             NVARCHAR(20)  NOT NULL,
...
CREATE TABLE amazon_webapp.asin_identifier_overrides (
    id            INT            IDENTITY(1,1) NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
...
CREATE TABLE amazon_webapp.pair_library_entries (          -- was pair_library
    id            INT            IDENTITY(1,1) NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
...
CREATE TABLE amazon_webapp.pair_library_ids (
    id            INT            IDENTITY(1,1) NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
    ...
    CONSTRAINT fk_pair_library_ids_entry
        FOREIGN KEY (asin) REFERENCES amazon_webapp.pair_library_entries(asin) ON DELETE CASCADE
);
...
CREATE TABLE amazon_webapp.brand_entities (                -- was brand_library
    id                  INT           IDENTITY(1,1) NOT NULL,
...
CREATE TABLE amazon_webapp.scans (
    id                  INT           IDENTITY(1,1) NOT NULL,
    name                NVARCHAR(500) NOT NULL,
...
CREATE TABLE amazon_webapp.scan_catalog_rows (
    id            INT            IDENTITY(1,1) NOT NULL,
    ...
    CONSTRAINT fk_scan_catalog_rows_scan
        FOREIGN KEY (scan_id) REFERENCES amazon_webapp.scans(id) ON DELETE CASCADE
);

-- Amazon/Keepa rows uploaded for a scan, keyed by ASIN.
CREATE TABLE amazon_webapp.scan_amazon_rows (
    id            INT            IDENTITY(1,1) NOT NULL,
    scan_id       INT            NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
    data_json     NVARCHAR(MAX)  NOT NULL,
...
CREATE TABLE amazon_webapp.scan_results (
    id            INT            IDENTITY(1,1) NOT NULL,
    scan_id       INT            NOT NULL,
    row_idx       INT            NOT NULL,
    upc           NVARCHAR(20)       NULL,
...
CREATE TABLE amazon_webapp.scan_candidates (
    id            INT            IDENTITY(1,1) NOT NULL,
    scan_id       INT            NOT NULL,
    row_idx       INT            NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
...
CREATE TABLE amazon_webapp.analytics_runs (
    id                     INT           IDENTITY(1,1) NOT NULL,
...
CREATE TABLE amazon_webapp.analytics_catalog_rows (
    id             INT           IDENTITY(1,1) NOT NULL,
    run_id         INT           NOT NULL,
    row_idx        INT           NOT NULL,
    data_json      NVARCHAR(MAX) NOT NULL,
    ...
    CONSTRAINT fk_analytics_catalog_rows_run
        FOREIGN KEY (run_id) REFERENCES amazon_webapp.analytics_runs(id) ON DELETE CASCADE
);
...
CREATE TABLE amazon_webapp.analytics_candidates (
    id            INT            IDENTITY(1,1) NOT NULL,
    run_id        INT            NOT NULL,
    row_idx       INT            NOT NULL,
    asin          NVARCHAR(20)   NOT NULL,
...
CREATE TABLE amazon_webapp.brand_analytics_runs (
    id                   INT           IDENTITY(1,1) NOT NULL,
    name                 NVARCHAR(500) NOT NULL,
    ...
    CONSTRAINT fk_brand_analytics_runs_library
        FOREIGN KEY (library_id) REFERENCES amazon_webapp.brand_entities(id) ON DELETE SET NULL
);

-- One discovered ASIN per brand run (core Brand Analytics results table).
CREATE TABLE amazon_webapp.brand_analytics_items (
    id             INT            IDENTITY(1,1) NOT NULL,
    run_id         INT            NOT NULL,
    ...
    CONSTRAINT fk_brand_analytics_items_run
        FOREIGN KEY (run_id) REFERENCES amazon_webapp.brand_analytics_runs(id) ON DELETE CASCADE
);
GO
...
CREATE INDEX ix_keepa_imports_imported_at      ON amazon_webapp.keepa_imports (imported_at);
CREATE INDEX ix_amazon_imports_imported_at     ON amazon_webapp.amazon_imports (imported_at);
CREATE INDEX ix_global_asin_caches_updated_at  ON amazon_webapp.global_asin_caches (updated_at);

-- attribute_caches: uq(upc, asin) covers upc; add the asin-only lookup.
CREATE INDEX ix_attribute_caches_asin          ON amazon_webapp.attribute_caches (asin);

-- verified_items: uq(upc, asin) covers upc; add asin lookup ("all pairs for ASIN").
CREATE INDEX ix_verified_items_asin            ON amazon_webapp.verified_items (asin);

-- blacklisted_pairs: uq(upc, asin) covers upc; add asin lookup.
CREATE INDEX ix_blacklisted_pairs_asin         ON amazon_webapp.blacklisted_pairs (asin);
...
CREATE INDEX ix_pair_library_entries_brand     ON amazon_webapp.pair_library_entries (brand);

-- pair_library_ids: uq(asin,id_type,identifier) covers the asin FK; the runtime
-- lookup is (id_type, identifier) → set of ASINs.
CREATE INDEX ix_pair_library_ids_lookup        ON amazon_webapp.pair_library_ids (id_type, identifier);

-- brand_entities: uq(name) covers name; Library tabs filter by type/parent.
CREATE INDEX ix_brand_entities_type            ON amazon_webapp.brand_entities (entity_type);
CREATE INDEX ix_brand_entities_parent          ON amazon_webapp.brand_entities (parent_manufacturer);

-- scans: run list ordering + status filtering.
CREATE INDEX ix_scans_created_at               ON amazon_webapp.scans (created_at DESC);
CREATE INDEX ix_scans_status                   ON amazon_webapp.scans (status);
...
CREATE INDEX ix_scan_amazon_rows_asin          ON amazon_webapp.scan_amazon_rows (asin);

-- scan_results: uq(scan_id,row_idx) covers the FK.
CREATE INDEX ix_scan_results_upc_asin          ON amazon_webapp.scan_results (upc, asin);  -- pair-store cross-ref
CREATE INDEX ix_scan_results_asin              ON amazon_webapp.scan_results (asin);
CREATE INDEX ix_scan_results_verdict           ON amazon_webapp.scan_results (scan_id, verdict);
...
CREATE INDEX ix_scan_candidates_scan_asin      ON amazon_webapp.scan_candidates (scan_id, asin);
CREATE INDEX ix_scan_candidates_asin           ON amazon_webapp.scan_candidates (asin);

-- analytics_runs: run list ordering + status filtering.
CREATE INDEX ix_analytics_runs_created_at      ON amazon_webapp.analytics_runs (created_at DESC);
CREATE INDEX ix_analytics_runs_status          ON amazon_webapp.analytics_runs (status);
...
CREATE INDEX ix_analytics_candidates_verdict_conf
    ON amazon_webapp.analytics_candidates (run_id, verdict, confidence DESC);
CREATE INDEX ix_analytics_candidates_ai        ON amazon_webapp.analytics_candidates (run_id, ai_verdict);
CREATE INDEX ix_analytics_candidates_rank      ON amazon_webapp.analytics_candidates (run_id, sales_rank);
CREATE INDEX ix_analytics_candidates_asin      ON amazon_webapp.analytics_candidates (asin);

-- brand_analytics_runs: run list ordering, status filter, library FK.
CREATE INDEX ix_brand_analytics_runs_created_at ON amazon_webapp.brand_analytics_runs (created_at DESC);
CREATE INDEX ix_brand_analytics_runs_status     ON amazon_webapp.brand_analytics_runs (status);
CREATE INDEX ix_brand_analytics_runs_library    ON amazon_webapp.brand_analytics_runs (library_id);
...
CREATE INDEX ix_brand_analytics_items_run_bsr   ON amazon_webapp.brand_analytics_items (run_id, bsr);
CREATE INDEX ix_brand_analytics_items_ai_fill   ON amazon_webapp.brand_analytics_items (run_id, ai_fill_status);
CREATE INDEX ix_brand_analytics_items_asin      ON amazon_webapp.brand_analytics_items (asin);
CREATE INDEX ix_brand_analytics_items_upc        ON amazon_webapp.brand_analytics_items (upc);
CREATE INDEX ix_brand_analytics_items_ean        ON amazon_webapp.brand_analytics_items (ean);
CREATE INDEX ix_brand_analytics_items_mpn        ON amazon_webapp.brand_analytics_items (mpn);
CREATE INDEX ix_brand_analytics_items_gtin       ON amazon_webapp.brand_analytics_items (gtin);
...
/* ===========================================================================
   ...
   =========================================================================== */
IF OBJECT_ID('amazon_webapp.scan_stats', 'V') IS NOT NULL
    DROP VIEW amazon_webapp.scan_stats;
GO
CREATE VIEW amazon_webapp.scan_stats AS
    SELECT
        scan_id,
    ...
    FROM amazon_webapp.scan_results
    GROUP BY scan_id;
GO
...
IF NOT EXISTS (SELECT 1 FROM amazon_webapp.settings WHERE [key] = 'threshold_verified')
    INSERT INTO amazon_webapp.settings ([key],[value]) VALUES ('threshold_verified','85');
IF NOT EXISTS (SELECT 1 FROM amazon_webapp.settings WHERE [key] = 'threshold_review')
    INSERT INTO amazon_webapp.settings ([key],[value]) VALUES ('threshold_review','35');
GO
...
INSERT INTO amazon_webapp.abbreviation_entries (category, abbr, full_form, added_by)
...
WHERE NOT EXISTS (SELECT 1 FROM amazon_webapp.abbreviation_entries e
